    ...
    <div className="p-6 h-full flex flex-col max-w-5xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <select 
            className="bg-card border border-gray-800 text-white text-sm rounded px-3 py-1.5 focus:outline-none focus:border-accent"
            value={filterType}
            onChange={(e) => {
          ...
          {filterType === 'ticker' && (
            <input 
              ...
              className="bg-card border border-gray-800 text-white text-sm rounded px-3 py-1.5 focus:outline-none focus:border-accent w-32 uppercase"
              value={filterValue}
      ...
      <div className="flex-1 overflow-y-auto hide-scrollbar pb-10 pr-2">
        {isLoading && !news ? (
          <div className="flex flex-col gap-4">
            {[1, 2, 3, 4, 5].map(i => (
              <div key={i} className="h-24 bg-card rounded-xl shimmer-bg border border-gray-800" />
            ))}
          </div>
        ) : news && news.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {news.map((item, idx) => (
              <a 
                ...
                className="group bg-card border border-gray-800 p-5 rounded-xl hover:border-accent transition-colors flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <span className="text-[10px] uppercase font-bold tracking-wider text-base bg-accent px-2 py-0.5 rounded">
                      {item.ticker}
                    </span>
                    <span className="text-muted text-[11px] font-medium">{item.source}</span>
                  </div>
                ...
                <div className="mt-4 pt-4 border-t border-gray-800/50 flex items-center justify-between">
                  <span className="text-muted text-[11px]">{formatTime(item.timestamp)}</span>
          ...
          <div className="text-center py-20 text-muted bg-card rounded-xl border border-gray-800 border-dashed">
